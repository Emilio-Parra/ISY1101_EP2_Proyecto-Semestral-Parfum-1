...
public class DespachoServiceImpl implements DespachoService{
    ...
    public Despacho updateDespacho(Long idDespacho, Despacho despacho) throws DespachoNotFoundException {
        return despachoRepository.findById(idDespacho).map(existingDespacho -> {
            ...
            existingDespacho.setTransportista(despacho.getTransportista());
            existingDespacho.setCodigoSeguimiento(despacho.getCodigoSeguimiento());
        ...
        }).orElseThrow(() -> new DespachoNotFoundException("Despacho de perfumería no encontrado con ID: " + idDespacho));
    }
    ...
    public void deleteDespacho(Long idDespacho) throws DespachoNotFoundException {
        ...
        if(!despacho.isPresent()){
            throw new DespachoNotFoundException("No existe despacho de perfumería con ID: " + idDespacho);
        }else {
    ...
    public Despacho findById(Long idDespacho) throws DespachoNotFoundException {
        Optional<Despacho> despacho = despachoRepository.findById(idDespacho);
        if(!despacho.isPresent()) throw new DespachoNotFoundException("No existe despacho de perfumería con ID: " + idDespacho);
        return despacho.get();
    }
}
